        ...
        if 'merlin_version' not in self.parameters:
            ...
            if not merlin.is_compatible(self.parameters['merlin_version']):
                raise merlin.IncompatibleVersionException(
                    f'Analysis task {self.analysisName} has already been created by MERlin ' +
                    f'version {self.parameters["merlin_version"]}, which is incompatible with the current ' +
                    f'MERlin version, {merlin.version()}')
        ...
        try:
            if self.is_running():
                raise AnalysisAlreadyStartedException(
                    f'Unable to run {self.analysisName} since it is already running')
            ...
            if self.is_complete() or self.is_error():
                raise AnalysisAlreadyStartedException(
                    f'Unable to run {self.analysisName} since it has already run')
...
class ParallelAnalysisTask(AnalysisTask):
    ...
    def run(self, fragmentIndex: int=None, overwrite=True) -> None:
        ...
        if fragmentIndex is None:
            ...
            logger.info(
                f'Beginning {self.get_analysis_name()} {fragmentIndex}')
            try:
                if self.is_running(fragmentIndex):
                    raise AnalysisAlreadyStartedException(
                        f'Unable to run {self.analysisName} fragment {fragmentIndex} since it is already ' +
                        'running')
                ...
                if self.is_complete(fragmentIndex) \
                    ...
                    raise AnalysisAlreadyStartedException(
                        f'Unable to run {self.analysisName} fragment {fragmentIndex} since it has already run')
                ...
                logger.info(f'Completed {self.get_analysis_name()} {fragmentIndex}')
