...
def _get_input_path(prompt):
    while True:
        ...
        if not pathString.startswith('s3://') \
            ...
            print(f'Directory {pathString} does not exist. Please enter a valid path.')
        else:
            return pathString
    ...
    if not args.generate_only:
        if args.analysis_task:
            ...
            if args.check_done:
                ...
                print(f'Running {args.analysis_task}')
...
def generate_analysis_tasks_and_snakefile(dataSet: dataset.MERFISHDataSet,
    ...
    print(f'Generating analysis tasks from {parametersFile.name}')
    ...
    print(f'Snakefile generated at {snakefilePath}')
