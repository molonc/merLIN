...
class PixelBasedDecoder(object):
    ...
    def decode_pixels(self, imageData: np.ndarray,
        ...
        # Debug: Check for NaN values that shouldn't exist in original working code
        if np.any(np.isnan(normalizedPixelTraces)):
            print(f"WARNING: Found {np.sum(np.isnan(normalizedPixelTraces))} NaN values in normalizedPixelTraces")
            print(f"scaledPixelTraces has NaN: {np.any(np.isnan(scaledPixelTraces))}")
            print(f"pixelMagnitudes has NaN: {np.any(np.isnan(pixelMagnitudes))}")
            print(f"pixelMagnitudes has zeros: {np.any(pixelMagnitudes == 0)}")
