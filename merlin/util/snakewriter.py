...
class SnakemakeRule(object):
    ...
    def _expand_as_string(self, taskName, indexCount) -> str:
        return 'expand(%s, g=list(range(%s)))' % (
            self._add_quotes(self._analysisTask.dataSet.analysis_done_filename(taskName, "{g}")), 
            indexCount)
        ...
        if isinstance(self._analysisTask, analysistask.ParallelAnalysisTask):
            fullString += \
                ('rule %sDone:\n\tinput: %s\n\toutput: %s\n\tmessage: %s\n\t'
                 + 'shell: %s\n\n') % (
                     self._analysisTask.get_analysis_name(),
                     self._clean_string(self._expand_as_string(self._analysisTask, self._analysisTask.fragment_count())),
                     self._add_quotes(self._clean_string(self._analysisTask.dataSet.analysis_done_filename(self._analysisTask))),
